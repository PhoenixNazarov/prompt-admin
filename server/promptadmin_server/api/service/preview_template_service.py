...
class PreviewTemplateService:
        ...
        if context:
            ...
            if mapping is not None:
                current_mapping_entity = [
                    i
                    for i in mapping_entity
                    if (
                        i.connection_name is None
                        or i.connection_name == mapping.connection_name
                    )
                    and (i.table is None or i.table == mapping.table)
                    and (i.field is None or i.field == mapping.field)
                    and (i.name is None or i.name == prompt.name)
                    and (i.mapping_id is None or i.mapping_id == mapping.id)
                    and (i.entity == "input")
                ]
    ...
    @staticmethod
    async def preview(
        prompt: str, context: dict[str, Any], connection: str | None = None
    ):
        ...
        if connection:
            ...
            async def render_template(template_key: str, **kwargs):
                ...
                return await templ_.render_async(
                    **context, **kwargs, render_template=render_template
                )
    ...
    async def execute(
        ...
        model_response = await build_model(model_service_info).execute(
            prompt, history, streamer
        )
        parsed_model_error = False
        if parsed_model_type:
            parsed_model = ParserOutputService().parse_for_json_schema(
                parsed_model_type, model_response.raw_text
            )
        ...
        return PromptExecute(
            ...
            messages=(
                model_response.messages
                if isinstance(model_response, AnthropicModelResponse)
                else []
            ),
        )

...
class PromptUnitTestService:
    ...
    async def process_sync_data(self, sync_data: SyncData, delay: int):
        if sync_data.id is None:
            return
        mapping_entities = await self.mapping_entity_service.find_by_entity_id(
            "sync_data", sync_data.id
        )
        if len(mapping_entities) <= 0:
            await self.sync_data_service.remove(sync_data)
            return
        mapping_entity = mapping_entities[0]
        ...
        view_params = (
            ...
            .filter(
                ViewParamsFilter(
                    field=Mapping.connection_name, value=mapping_entity.connection_name
                )
            )
            .build()
        )
    ...
    async def process_sync_data_name(
        ...
        if sync_data.id is None:
            return
        mapping_entity_disable = (
            await self.mapping_entity_service.find_by_prompt_entity(
                mapping.connection_name, mapping.table, mapping.field, name, "disable"
            )
        )
        ...
        if unit_test is None:
            unit_test = UnitTest(sync_data_id=sync_data.id, name=name)
    ...
    async def preview(self, unit_test: UnitTest, sync_data: SyncData, mapping: Mapping):
        ...
        try:
            ...
            prompt = await self.prompt_load_service.load_mapping_name(
                mapping, unit_test.name
            )
            preview = await self.preview_template_service.preview(
                prompt, context, mapping.connection_name
            )
    ...
    async def execution(self, unit_test: UnitTest, sync_data: SyncData):
        ...
        try:
            execution = await self.preview_template_service.execute(
                model_service_info=ModelServiceInfo.model_validate_json(
                    sync_data.service_model_info
                ),
                prompt=unit_test.test_preview,
                history=[
                    Message.model_validate(i)
                    for i in json.loads(sync_data.history_context_default)
                ],
                parsed_model_type=(
                    json.loads(sync_data.parsed_model_type)
                    if sync_data.parsed_model_type
                    else None
                ),
            )

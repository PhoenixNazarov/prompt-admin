...
class AccessPermissionService:
    ...
    async def get_config(self, account: Account) -> list[Permission]:
        if account.id is None:
            raise TypeCheckException()
        return await self.permission_service.find_by_key(
            Permission.account_id, account.id
        )

    async def set_permission(
        self, account_id: int, key: str, value: int, project: str | None
    ) -> Permission:
        permission = await self._find_permission(account_id, key, project)
        if permission:
            permission.value = value
        else:
            permission = Permission(
                account_id=account_id, key=key, value=value, project=project
            )

        return await self.permission_service.save(permission)
